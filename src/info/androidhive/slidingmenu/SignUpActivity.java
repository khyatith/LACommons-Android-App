...
public class SignUpActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final EditText mobilenumber=(EditText) findViewById(R.id.mobileedittext);
		...
		Button signupbutton=(Button) findViewById(R.id.signupbutton);
		 final DBHelper db=new DBHelper(getApplicationContext());
			...
			public void onClick(View v) {
				...
				List<Customers> allTags =db.getAllCustomers();
				...
				final String cpwd=confirmpassword.getText().toString();
				final String contactnumber=mobilenumber.getText().toString();
				TextView errortextview=(TextView) findViewById(R.id.errortextview);
				String emailPattern = "[a-zA-Z0-9._-]+@[a-z]+\\.+[a-z]+";
				
				if(lname.trim().length()==0 || fname.trim().length()==0 || uname.trim().length()==0 || pwd.trim().length()==0 || cpwd.trim().length()==0)
				{
					Toast.makeText(getApplicationContext(), "all required fields need to be filled", Toast.LENGTH_SHORT).show();
				}
					else if(contactnumber.trim().length()>0 && contactnumber.trim().length()<10)
					{
					...
					else
					{
						...
						for (Customers tag : allTags) {
				            final String existingusername=tag.getcustomerusername();
						
				           if(existingusername==uname)
				           {
				        	   Toast.makeText(getApplicationContext(), "The username already exists in our records! Please Sign In if you are already a user!", Toast.LENGTH_SHORT).show();
				           }
						
				           else
				           {
				Customers customers=new Customers(fname,lname,uname,pwd,contactnumber);
				//DBHelper db;
				//db=new DBHelper(getApplicationContext());
				Long cid=db.createCustomers(customers);
				Log.d("Inserted Record", ""+cid);
				if(cid>0)
				{
					
					errortextview.setText("Successfull Registeration!Please Login Before you book tickets!");
				...
				else
				{
					...
					alertDialogBuilder
					.setMessage("A system error occurred while registering.Please try again later after sometime!")
					.setCancelable(false)
